...
| V2-010 External historical blind test | BLOCKED_DATA; GLOBAL_ETF_USD_FREE_RESEARCH_AUDIT_SELECTED | CNY/CSI500 contracts remain `PENDING_SOURCE_APPROVAL`. The selected GLOBAL_ETF_USD path has an offline qualification receipt `0b982…a6fa96` that records the seven missing artifacts (approval, raw manifest, calendar, corporate actions, PIT membership, costs and reproduction). The next zero-cost step reuses the existing Yahoo raw-response archive and issuer official records only for a non-promoting crosscheck; EODHD and other paid providers are deferred. No source capture or historical return test has run. See V2_010_GLOBAL_ETF_USD_INITIAL_QUALIFICATION.md and V2_010_GLOBAL_ETF_USD_PROVIDER_OPTIONS.md. |
