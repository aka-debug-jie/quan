...
| V2-010 External historical blind test | BLOCKED_DATA; GLOBAL_ETF_USD_FREE_SOURCE_CAPTURE_AND_TEMPLATES_COMPLETED | CNY/CSI500 contracts remain `PENDING_SOURCE_APPROVAL`. The selected GLOBAL_ETF_USD path has an offline qualification receipt `0b982…a6fa96` that records the seven missing artifacts (approval, raw manifest, calendar, corporate actions, PIT membership, costs and reproduction). A zero-cost audit SHA-verified all seven existing Yahoo raw archives, receipt `cf3d54…674f7`, and a constrained public-source capture archived the NYSE, State Street, iShares and Invesco bodies, receipt `13232e…8f29`. Source-bound calendar, issuer-action and static-universe templates are now available but are intentionally rejected as evidence until normalized records exist; the route remains non-promoting. EODHD and other paid providers remain deferred. No historical return test has run. See V2_010_GLOBAL_ETF_USD_FREE_SOURCE_CAPTURE_RESULT.md. |
