...
| V2-010 External historical blind test | BLOCKED_DATA; GLOBAL_ETF_USD_FREE_RESEARCH_AUDIT_COMPLETED | CNY/CSI500 contracts remain `PENDING_SOURCE_APPROVAL`. The selected GLOBAL_ETF_USD path has an offline qualification receipt `0b982…a6fa96` that records the seven missing artifacts (approval, raw manifest, calendar, corporate actions, PIT membership, costs and reproduction). A zero-cost audit then SHA-verified all seven existing Yahoo raw archives and their coverage, receipt `cf3d54…674f7`, but issuer-action and official-calendar crosschecks remain `NOT_PROVIDED`; it is explicitly non-promoting. EODHD and other paid providers remain deferred. No source capture or historical return test has run. See V2_010_GLOBAL_ETF_USD_FREE_RESEARCH_AUDIT_RESULT.md. |
