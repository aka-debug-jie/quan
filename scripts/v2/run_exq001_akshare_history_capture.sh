...
if [[ ${EUID} -ne 0 ]]; then
  echo "run as root: sudo bash scripts/v2/run_exq001_akshare_history_capture.sh" >&2
  exit 2
fi
...
"$repo_root/.venv/bin/python" -m quant_stack_v2.exq_akshare_history_capture \
  --development-root /srv/quant-v2/development \
